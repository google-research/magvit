...
class TrainerTest(parameterized.TestCase):
  """Test cases for the maskgvt trainer."""
  ...
  def get_config(self):
    config = maskgvt_ucf101_config.get_config(config_str='B-runlocal')
    ...
    config.tasks = ('full_generation', 'frame_prediction')
  ...
  def test_train(self):
    ...
    train_state, _ = maskgvt_trainer.train(
        rng=rng,
        config=config,
        dataset=dataset,
